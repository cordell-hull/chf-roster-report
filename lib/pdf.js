...
export class PDFGenerator {
  ...
  _wrapText(text, maxWidth, fontSize) {
    ...
    for (const word of words) {
      ...
      if (this.font.widthOfTextAtSize(test, fontSize) > maxWidth && cur) {
        lines.push(cur);
        cur = word;
      } else {
        cur = test;
  ...
  drawTable(headers, rows, colWidths, fontSize = this.fonts.caption) {
    ...
    for (let r = 0; r < rows.length; r++) {
      // Calculate wrapped lines for each cell to determine row height
      const cellLines = rows[r].map((cell, c) => {
        const text = this._sanitize(cell);
        const maxW = colWidths[c] - 4;
        return this._wrapText(text, maxW, fontSize);
      });

      const maxLines = Math.max(...cellLines.map(l => l.length), 1);
      const actualRowH = Math.max(rowH, maxLines * lineH + cellPad * 2);

      if (y - actualRowH < this.margins.bottom) {
        this.addNewPage();
        y = this.currentY;
        _drawHeader();
      }
      ...
      if (r % 2 === 0) {
        this.currentPage.drawRectangle({
          x: startX, y: y - actualRowH, width: this.contentWidth, height: actualRowH,
      ...
      for (let c = 0; c < cellLines.length; c++) {
        let lineY = y - cellPad - fontSize;
        for (const line of cellLines[c]) {
          this.currentPage.drawText(line, {
            x: x + 2, y: lineY, size: fontSize, font: this.font,
  ...
  async generate(report, onProgress = () => {}) {
    ...
    const teacherHeaders = ['No', 'Last Name', 'First Name', 'Email', 'M/F', 'Yr', 'Citizenship', 'Residence', 'Level', 'State', 'Salary', 'Tenure'];
    const teacherWidths = [22, 70, 65, 110, 28, 25, 65, 60, 52, 30, 55, 150];

    const teacherRows = report.teachers.map((t, i) => [
      String(i + 1),
      t.lastName, t.firstName, t.email, t.gender,
      t.programYear, t.nationality, t.countryOfResidence,
      t.level, t.state, t.yearlySalary,
    ...
    if (report.relatives.length > 0) {
      ...
      const relHeaders = ['Last Name', 'First Name', 'Middle', 'Relationship', 'Teacher', 'Citizenship', 'Residence'];
      const relWidths = [110, 100, 80, 90, 150, 100, 102];
