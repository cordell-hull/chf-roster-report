...
async function _handleTeacherPdfImport(e) {
  const file = e.target.files[0];
  if (!file) return;
  const errorEl = document.getElementById('importTeachersError');
  errorEl.textContent = '';

  try {
    const data = await importReportFromPDF(file);
    if (!data || !data.teachers || data.teachers.length === 0) {
      errorEl.textContent = 'No teacher data found in this PDF. Only recruitment report PDFs generated by this app can be imported.';
      e.target.value = '';
      return;
    }

    const existingEmails = new Set(report.teachers.map(t => t.email.toLowerCase()));
    const available = data.teachers.filter(t =>
      t.email && !existingEmails.has(t.email.toLowerCase())
    );

    if (available.length === 0) {
      errorEl.textContent = 'All teachers in this PDF have already been imported.';
      e.target.value = '';
      return;
    }

    // Use the same import modal
    const list = document.getElementById('importTeacherList');
    list.innerHTML = available.map((t, i) => `
      <label class="checkbox-item import-check">
        <input type="checkbox" value="${i}" checked>
